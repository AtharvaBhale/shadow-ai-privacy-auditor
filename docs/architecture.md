...
## Tier 2: Browser Extension (ChatGPT Guard)

A Chrome extension (`extension/`) that watches ChatGPT's message composer directly on chatgpt.com and warns before sending sensitive content — combining the brief's "browser extension" and "direct integration with AI platforms" stretch goals into one deliverable, since building them separately would be redundant (the extension's whole purpose *is* direct platform integration).

**Architecture:** a Manifest V3 content script (`content.js`) injects into chatgpt.com, uses a `MutationObserver` plus input-event listeners to detect composer changes (ChatGPT is a single-page app, so elements load and change dynamically), and runs `detectors.js` against the current draft text on a 400ms debounce. Findings render in a floating panel; Send (both the button and Enter key) is intercepted via capture-phase listeners, and if risky content is present, the user gets a confirmation dialog before the message actually sends — a soft block, not a hard block.

**Detection scope — a deliberate, disclosed limitation, not an oversight:** browser extensions run JavaScript, not Python. `detectors.js` is a hand-ported JavaScript reimplementation of the regex/validation detectors from `src/engine.py` — contact info, government/financial identifiers (with Luhn validation), credentials (with Shannon entropy validation), medical info, employee/client IDs, and confidential-org keywords, including the Spanish multilingual patterns. **NAME detection is not available in the extension**, because it depends on the spaCy NER model, which cannot run inside a browser content script without standing up a separate API backend to send the user's draft text to — something this project deliberately avoided everywhere else, since sending private text to any external service undermines the tool's core purpose. Rather than compromise that principle just for the extension, NAME detection is explicitly out of scope here, and the extension's own panel tells the user this directly.

**Verification:** `detectors.js` was tested against the same 12 non-NER test cases from `test_cases.py`, run standalone via Node.js, before being wired into the content script — 12/12 passing, confirming the JS port produces identical categorization to the Python regex detectors it mirrors.